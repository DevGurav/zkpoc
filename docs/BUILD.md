...
| Q7 | Can mixing a memory-hard KDF into `merkle.js#hashRow`'s commitment close (or narrow) the Q1 gap without changing the underlying GEMM kernel or the useful-work claim? What does that cost `commitFullResult()`? | Mitigates ADR-0013 | **Partially resolved.** Built, tested, measured (`bench/memory_hard_overhead.js`) — cost is real and scales linearly with shard size; no buffer size tested is both plausibly GPU-resistant and practically fast at per-row granularity. Whether it actually narrows the gap remains unverified (no GPU in this environment to check against). Not deployed. [ADR-0016](adr/0016-memory-hard-commitment-mitigation.md) |
| Q8 | Would folding memory-hardness in once per submission (e.g. into the Merkle root) rather than once per row make the cost constant instead of linear in shard size — and is the resulting weaker per-row deterrence argument still worth it? | Alternative to Q7's per-row approach | Open — named in [ADR-0016](adr/0016-memory-hard-commitment-mitigation.md), not designed or built |
